...
# 输入x代表默认的torchvision返回的类别值，class_count 类别值为10
# torch.eye(x) 生成对角线全部为1，其余全部为零的二维数组
# x用于控制输出第x个数值为1其余为0的大小为10的数组
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=2)
        self.bn = nn.BatchNorm2d(128)
        self.fc = nn.Linear(128*6*6, 1) # 输出一个0~1之间的概率值
...
# 定义可视化函数
def generate_save_images(model, epoch, label_input, noise_input):
    prediction = np.squeeze(model(label_input, noise_input).cpu().numpy())
    fig = plt.figure(figsize=(4, 4))
    for i in range(prediction.shape[0]):
        plt.subplot(4, 4, i+1)
        plt.imshow((prediction[i]+1)/2)
        plt.axis("off")
    plt.savefig("./CGAN_MINST_results/{}.png".format(epoch))
...
for epoch in range(30):
    ...
    for step, (img, label) in enumerate(dataloader):
        ...
        d_real_loss.backward()  # 判别器对真实的数据求梯度
        ...
        g_loss = loss_func(fake_output, torch.ones_like(fake_output,device=device))
        g_loss.backward()
        g_optim.step()
    ...
    with torch.no_grad():
        d_epoch_loss /= count
        g_epoch_loss /= count
        print("第{}次Epoch, 生成器损失{}， 判别器损失{}".format(epoch+1, g_epoch_loss, d_epoch_loss))
        generate_save_images(gen, epoch, label_seed_onehot, noise_seed)
